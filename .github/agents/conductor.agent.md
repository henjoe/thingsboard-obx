...
3. **Synthesize Findings**: Collect and analyze the research output to identify key requirements, constraints, and potential implementation approaches. Ask follow-up questions to the planning-subagent if any critical information is missing or unclear.
...
5. **Present Plan to User**: Share the plan synopsis in chat, highlighting any open questions or implementation options. Ask clarification questions if needed to resolve uncertainties before finalizing the plan and for each question, provide 2-3 options if there are multiple approaches.
...
7. **Write Plan File**: Once approved, write the plan to `plans/<task-name>-plan.md`.
